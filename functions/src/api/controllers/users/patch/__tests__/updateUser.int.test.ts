...
import { app } from '../../../../../';
import { fakeBody, fakeBodyWithId } from '../../__mocks__/updateUser';
...
describe('updateUser', () => {
  ...
  it('should send a 200 and the user when it is updated', async () => {
    const id = 1;
    const response = await supertest(app).patch(`/users/${id}`).send(fakeBody);
  ...
  it('should send a status 404 and a message if there is not a user with this id', async () => {
    ...
    const response = await supertest(app).patch(`/users/${id}`);
